...
static int Wireless_dma_write(struct WirelessDeviceState *wd, struct Wireless_Data_Detail *data)
{
    printf("wireless device : wirte data to dma : host addr %08x data length %08x \n", data->host_addr, data->data_length);
    struct Wireless_DMA_Detail *dma_detail = &wd->wireless_dma_detail;
    if (data->data_length > dma_detail->dma_max_mem_size ||
        (data->data_length + dma_detail->dma_data_length) > dma_detail->dma_max_mem_size ||
        (dma_detail->dma_node_count_size != 0 && dma_detail->dma_node_count + 1 < dma_detail->dma_node_count_size))
    { // 加法运算没有考虑溢出，因此dma数据区域不能过大
...
static int Wirelsee_dma_read_old(struct WirelessDeviceState *wd)
{
